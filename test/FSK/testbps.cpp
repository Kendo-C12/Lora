...
// Packet settings
const int PACKET_SIZE = 250;       // Bytes per packet
byte packet[PACKET_SIZE];      // Buffer for packet data
...
void setup() {
  ...
  for(int i = 0;i < PACKET_SIZE;i++) packet[i] = byte('A');
...
void loop() {
  ...
  if(txflag){
    txflag = false;
    inTx = false;
    // Serial.println("Real Time on Air: " + String(micros() - last_finish_tx));
    last_finish_tx = micros();
  ...
  if(!inTx && micros() - last_finish_tx > tx_cycle) {
    inTx = true;
    last_finish_tx = micros();
    int state = radio.startTransmit(packet,PACKET_SIZE);
    if (state == RADIOLIB_ERR_NONE) {
