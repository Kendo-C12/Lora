...
constexpr struct
{
    ...
    float bitRate = 300; // kHz or kbps         must be 0.5 to 300 kbps
    float freqDev = bitRate/2; // kHz                 must be lower than 200 kHz
    float RxBw = 467.0; // bitRate + 2*freqDev + 10; // kHz                     must be 2.6 to 250 kHz
    int8_t power = 10; // dB
...
void loop() {
  ...
  if(rxFlag) {
    ...
    Serial.println("Receive Packet size: " + String(numBytes));
