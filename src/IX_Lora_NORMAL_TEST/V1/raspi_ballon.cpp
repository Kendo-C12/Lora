...
void loop(){
  ...
  if(!inTx) {
    if(!packet.empty()){
      ...
      if(lenChunk > 0) {
          ...
          if(lenChunk == 1){ // GPS
            lenChunk = snprintf(apogee_packet, sizeof(apogee_packet), "GS,%.6f,%.6f,%.6f", lat, lon, altFiltered);
  ...
  if (millis() - last.log > interval.log){
    ...
    last.log = millis();
    lenChunk = snprintf(apogee_packet, apogee_size, "GS,%.6f,%.6f,%.6f", lat, lon, altFiltered);

    Serial.print("LENAPOGEE");
    Serial.println(lenChunk);
    // Serial.println("===============STATE===============");
  ...
  if(ENABLE_SENSOR){
    ...
    if(millis() - last.gps > interval.gps){
      ...
      if (max10s.getPVT()){
        ...
        alt = max10s.getAltitude()/float(10000 * 1000);
